...
function rg_mcomp_banner_listing_page()
{
	global $wpdb;
	...
	$uploaddir = $base_dir.'/revglue/mobile-comparison/banners/';
	$uploadurl = $base_url.'/revglue/mobile-comparison/banners/';
	$placements = rg_banners_places();
	// pre($placements);
	...
	if ( $_SERVER['REQUEST_METHOD'] === 'POST' )
	{
		...
		if( is_uploaded_file( $_FILES['rg_banner_image_file']['tmp_name'] ) )
		{

			$path = sanitize_file_name($_FILES['rg_banner_image_file']['name']) ;
			//$path = $_FILES['rg_banner_image_file']['name'];
			$ext = pathinfo($path, PATHINFO_EXTENSION);
			$title = uniqid() . '.' . $ext;
			//$tmpfile = $_FILES['rg_banner_image_file']['tmp_name'];
	...
	$sql = "SELECT *FROM $banner_table ORDER BY rg_id DESC";				
	...
	?><div class="rg-admin-container add_banner_div <?php esc_html_e( $hide_form ) ?>">
		...
		<table class="form-table">
			...
			<tr id="rg_mcomp_banner_image_url" <?php echo ( empty ( $image_url ) ? 'style="display:none;"' : '' ); ?>>
				<th>
			...
			<tr id="rg_mcomp_banner_image_upload" <?php echo ( ! empty ( $image_url ) ? 'style="display:none;"' : '' ); ?>>
				<th>
	...
	<div class="rg-admin-container list_banner_div <?php esc_html_e( $hide_table ) ?>">
		...
		<div style="clear:both;"></div>
		<hr/>
			<div class="text-right">you can search by placement</div>
